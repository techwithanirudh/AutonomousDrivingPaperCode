...
def getFaceImage():
    ...
    if faceImg.status_code == 200:
        with open(facePath, 'wb') as imgFile:
            faceImg.raw.decode_content = True
            shutil.copyfileobj(faceImg.raw, imgFile)
...
def main():
    ...
    cvArea = cvWidth * cvHeight
    print(cvWidth, cvHeight, cvArea)
            ...
            for face_landmarks in results.multi_face_landmarks:
                ...
                if len(landmark_target_ocv) > 0:
                    ...
                    if process == True:
                        for i in range(0, int(len(tmp.TRIANGULATION) / 3)):
                            ...
                            warped_triangle = cv2.warpAffine(
                                cropped_triangle, M, (w, h), borderMode=cv2.BORDER_REPLICATE)
                            ...
                            img2_new_face[y: y + h, x: x +
                                          w] = img2_new_face_rect_area
                    ...
                    for face_landmarks in results.multi_face_landmarks:
                        ...
                        # This code is not helped at all by Sriram...
                        print((cx_min, cy_min), (cx_max, cy_max), (cx_max - cx_min, cy_max - cy_min))
                        faceArea = (cx_max - cx_min) * (cy_max - cy_min)
                        percentage = (faceArea / cvArea) * 100
                        cv2.putText(out_image,("Surface Area: " + str(round(percentage, 2)) + "%"), (0,30), cv2.FONT_HERSHEY_SIMPLEX, 1, 255)
                    
                        cv2.rectangle(
                            out_image, (cx_min, cy_min), (cx_max, cy_max), (255, 255, 0), 2
                        )
